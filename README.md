...
- **Shielded funds are guarded differently from the rest of your wallet.** Everywhere else in this app, your keys stay in the Rust backend and the interface only ever sees public data. That still holds for the raw key here — proof generation is a JavaScript and WASM library driven through the SDK's external-signer entry points, so the secret key itself never reaches the interface. But the signature it *does* receive is what the pool's spending key is derived from, and a withdrawal is authorised by that key rather than by a wallet signature. In practice that means the code running in the app window holds enough to move your shielded balance, and it cannot be otherwise while the prover runs there. A stolen copy of that credential stays valid after you lock the vault and after you change your password; only moving the funds and using a different wallet revokes it.

  The consequence worth internalising: **the shielded balance is only as safe as this app's JavaScript dependencies.** Your ordinary SOL is protected even against that; funds in the pool are not. Keep the amount there to what you would accept losing to a bad npm release. The note encryption key is otherwise the same one Privacy Cash's own web app derives, so the same wallet shows the same private balance in either.
